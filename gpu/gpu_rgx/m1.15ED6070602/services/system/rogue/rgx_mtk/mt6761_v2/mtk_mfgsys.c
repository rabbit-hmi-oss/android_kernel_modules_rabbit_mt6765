...
static void MTKEnableMfgClock(IMG_BOOL bForce)
{
	...
	gpufreq_power_control(POWER_ON);
#endif
	if (gpu_debug_enable)
...
#ifdef MTK_GPU_DVFS
	buck_state = gpufreq_power_control(POWER_OFF);
#endif
...
static IMG_BOOL MTKDoGpuDVFS(IMG_UINT32 ui32NewFreqID, IMG_BOOL bIdleDevice)
{
	...
	if ((eResult == PVRSRV_OK) || (eResult == PVRSRV_ERROR_RETRY)) {
		...
		gpufreq_commit(TARGET_GPU, ui32NewFreqID);
		ui32GPUFreq = gpufreq_get_freq_by_idx(TARGET_GPU, ui32NewFreqID);
	...
	if (gpu_debug_enable)
		...
		ui32GPUFreq, gpufreq_get_cur_volt(TARGET_GPU));
#endif
	...
	GED_DVFS_COMMIT_TYPE eCommitType, int *pbCommited)
{
	...
	if (psDevNode) {
		...
		if ((eResult == PVRSRV_OK) || (eResult == PVRSRV_ERROR_RETRY)) {
			...
			ui32CurFreqID = gpufreq_get_cur_oppidx(TARGET_GPU);
			ui32GPUFreq =
			gpufreq_get_freq_by_idx(TARGET_GPU, ui32CurFreqID);
			gpu_freq = ui32GPUFreq;
...
{
	int i32MaxLevel = (int)(gpufreq_get_opp_num(TARGET_GPU) - 1);
	...
	for (i = 0; i <= i32MaxLevel; i++) {
		gpu_freq = gpufreq_get_freq_by_idx(TARGET_GPU, i);
	...
	gpu_freq = gpufreq_get_freq_by_idx(TARGET_GPU, ui32NewFreqID);
...
static void MTKFreqInputBoostCB(unsigned int ui32BoostFreqID)
{
	...
	if (ui32BoostFreqID < gpufreq_get_cur_oppidx(TARGET_GPU)) {
...
static void MTKFreqPowerLimitCB(unsigned int ui32LimitFreqID)
{
	...
	if (ui32LimitFreqID > gpufreq_get_cur_oppidx(TARGET_GPU)) {
...
{
	int i32MaxLevel = (int)(gpufreq_get_opp_num(TARGET_GPU) - 1);
	int i32CurFreqID = (int)gpufreq_get_cur_oppidx(TARGET_GPU);
...
static void MTKDVFSTimerFuncCB(void *pvData)
{
	...
	if (g_iSkipCount > 0) {
		...
		gpu_power = 0;
#endif
		MTKCalGpuLoading(&gpu_loading, &gpu_block, &gpu_idle, NULL);
		OSLockAcquire(ghDVFSLock);
...
#ifdef MTK_GPU_DVFS
				gpufreq_commit(TARGET_GPU, g_ui32_unsync_freq_id);
#endif
				g_bUnsync = IMG_FALSE;
			}
...
static void MTKSetBottomGPUFreq(unsigned int ui32FreqLevel)
{
	...
	ui32MaxLevel = gpufreq_get_opp_num(TARGET_GPU) - 1;
	...
	gpu_bottom_freq = gpufreq_get_freq_by_idx(TARGET_GPU, g_bottom_freq_id);

	if (g_bottom_freq_id < gpufreq_get_cur_oppidx(TARGET_GPU))
...
{
	return gpufreq_get_opp_num(TARGET_GPU);
}
...
static void MTKCustomBoostGpuFreq(unsigned int ui32FreqLevel)
{
	...
	gpu_cust_boost_freq
	= gpufreq_get_freq_by_idx(TARGET_GPU, g_cust_boost_freq_id);

	if (g_cust_boost_freq_id < gpufreq_get_cur_oppidx(TARGET_GPU))
...
static void MTKCustomUpBoundGpuFreq(unsigned int ui32FreqLevel)
{
	...
	= gpufreq_get_freq_by_idx(TARGET_GPU, g_cust_upbound_freq_id);

	if (g_cust_upbound_freq_id > gpufreq_get_cur_oppidx(TARGET_GPU))
...
{
	unsigned int ui32MaxLevel = gpufreq_get_opp_num(TARGET_GPU) - 1;
...
static int MTKMFGOppUpdate(int ui32ThrottlePoint)
{
	...
	if (bNotReady) {
		ui32OPPTableSize = gpufreq_get_opp_num(TARGET_GPU);
		gpasOPPTable
		= (IMG_OPP *)OSAllocZMem(sizeof(IMG_OPP) * ui32OPPTableSize);

		for (i = 0; i < ui32OPPTableSize; i++) {
			gpasOPPTable[i].ui32Volt = __gpufreq_get_vgpu_by_idx(i);
			gpasOPPTable[i].ui32Freq
			= gpufreq_get_freq_by_idx(TARGET_GPU, i) * 1000;
		}
...
PVRSRV_ERROR MTKMFGSystemInit(void)
{
	...
	g_sys_dvfs_time_ms = 0;
	g_bottom_freq_id = gpufreq_get_opp_num(TARGET_GPU) - 1;
	gpu_bottom_freq = gpufreq_get_freq_by_idx(TARGET_GPU, g_bottom_freq_id);
	g_cust_boost_freq_id = gpufreq_get_opp_num(TARGET_GPU) - 1;
	gpu_cust_boost_freq =
	gpufreq_get_freq_by_idx(TARGET_GPU, g_cust_boost_freq_id);
